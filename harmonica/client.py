...
class HarmonicaClient:
    ...
    def __init__(self, api_key: str | None = None, base_url: str | None = None):
        ...
        self.base_url = (base_url or os.getenv("HARMONICA_API_URL", _DEFAULT_BASE_URL)).rstrip("/")
    ...
    def _request(self, method: str, path: str, **kwargs) -> Any:
        for attempt in range(_MAX_RETRIES):
            ...
            if resp.status_code >= 400:
                raise HarmonicaError(f"HTTP {resp.status_code}: {resp.text}")
            return resp.json()
    ...
    def create_session(
        ...
        """Create a new Harmonica session. Returns session dict with id and URL."""
        body: dict[str, Any] = {"topic": topic, "goal": goal, "cross_pollination": cross_pollination}
        if prompt:
            body["prompt"] = prompt
        if questions:
    ...
    def list_sessions(self, status: str | None = None, keyword: str | None = None) -> list[dict]:
        params: dict[str, str] = {}
        if status:
    ...
    def update_session(self, session_id: str, **fields) -> dict:
        return self._request("PATCH", f"/sessions/{session_id}", json=fields)

    def search_sessions(self, query: str) -> list[dict]:
        return self._request("GET", "/sessions", params={"keyword": query})

    # --- Participants & Responses ---

    def list_participants(self, session_id: str) -> list[dict]:
        return self._request("GET", f"/sessions/{session_id}/participants")
    ...
    def get_responses(
        ...
        return self._request("GET", f"/sessions/{session_id}/responses", params=params)

    def get_questions(self, session_id: str) -> list[dict]:
        return self._request("GET", f"/sessions/{session_id}/questions")
    ...
    def submit_questions(self, session_id: str, answers: list[dict]) -> dict:
        return self._request("POST", f"/sessions/{session_id}/questions", json={"answers": answers})

    # --- Summaries ---

    def generate_summary(self, session_id: str, prompt: str | None = None) -> dict:
        body: dict[str, Any] = {}
        if prompt:
            body["prompt"] = prompt
        return self._request("POST", f"/sessions/{session_id}/summary", json=body)

    def get_summary(self, session_id: str) -> dict:
        return self._request("GET", f"/sessions/{session_id}/summary")
